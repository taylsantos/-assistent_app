...
class _MainHomeScreenState extends State<MainHomeScreen> {
  ...
  void _processLocalAI(String input) {
    ...
    if (isGain && val > 0) {
      ...
      if (_selectedPersonality == 'sincero') {
        reply = "Aê! R\$ ${val.toStringAsFixed(2)} de entrada ($description). Finalmente uma notícia boa! Vê se não gasta tudo de uma vez, tá? 🎭";
      } else if (_selectedPersonality == 'motivador') {
        reply = "BOA DEMAIS! 🔥 R\$ ${val.toStringAsFixed(2)} caindo na conta ($description)! Você está construindo sua liberdade financeira! 🚀";
      } else if (_selectedPersonality == 'executivo') {
        reply = "Entrada confirmada: R\$ ${val.toStringAsFixed(2)} ($category). Saldo atualizado para R\$ ${saldo.toStringAsFixed(2)}. 📊";
      } else {
  ...
  Widget _buildPersonalityChip(String label, String value) {
    bool selected = _selectedPersonality == value;
    Color primaryColor = Theme.of(context).primaryColor;
    ...
    return Padding(
      ...
      child: ChoiceChip(
        ...
        backgroundColor: Theme.of(context).cardColor,
        labelStyle: TextStyle(color: selected ? Colors.black : Theme.of(context).textTheme.bodyLarge?.color),
        onSelected: (val) {
          if (val) {
            setState(() {
